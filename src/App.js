...
class App extends Component {
	...
	componentDidMount () {
		const self = this;
		fetch("https://hacker-news.firebaseio.com/v0/beststories.json").then(res => res.json()).then((response) => {
	...
	render() {
		if(this.state.storyDetails.length>0) {
			return (
			  <div>
			    <h1>HackerNews REST API </h1>
			  	<NewsList properties={this.state.storyDetails} />		  	
			  </div>
			);
		} else {
